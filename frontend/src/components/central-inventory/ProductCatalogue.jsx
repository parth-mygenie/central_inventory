        ...
        <>
          ...
          <Tabs defaultValue="foods">
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="addon-recipes" data-testid="tab-addon-recipes">Addon Recipes</TabsTrigger>
            </TabsList>
            <TabsContent value="foods"><FoodsTab /></TabsContent>
            <TabsContent value="categories"><FoodCategoriesTab /></TabsContent>
            <TabsContent value="recipes"><RecipeCatalogue embedded /></TabsContent>
            <TabsContent value="addons"><AddonsTab /></TabsContent>
            <TabsContent value="addon-recipes"><AddonRecipeCatalogue embedded /></TabsContent>
          </Tabs>
        </>
      )}
    </div>
  ...
  return (
    <>
      <div className="flex items-center gap-2 mb-3">
        ...
        <Button size="sm" onClick={() => { setEditFood(null); setSheetOpen(true); }} data-testid="add-food-btn"><Plus className="h-4 w-4 mr-1" />Add Food</Button>
      </div>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-[400px] sm:w-[440px] flex flex-col">
        ...
        <div className="flex-1 space-y-4 py-4">
          <div><Label className="text-xs">Name *</Label><Input value={name} onChange={e => setName(e.target.value)} className="h-9 text-sm mt-1" data-testid="food-name" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-xs">Category</Label>
              <Select value={catId} onValueChange={setCatId}><SelectTrigger className="h-9 text-sm mt-1" data-testid="food-cat"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{categories.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.name}</SelectItem>)}</SelectContent></Select></div>
            <div><Label className="text-xs">Price (₹) *</Label><Input type="number" value={price} onChange={e => setPrice(e.target.value)} className="h-9 text-sm mt-1" data-testid="food-price" /></div>
          </div>
          <div><Label className="text-xs">Description</Label><Input value={desc} onChange={e => setDesc(e.target.value)} className="h-9 text-sm mt-1" data-testid="food-desc" /></div>
          {food && (
        ...
        <SheetFooter className="border-t pt-4">
          ...
          <Button onClick={save} disabled={saving || !name.trim() || !price} size="sm" data-testid="save-food-btn">{saving && <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />}Save</Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
  );

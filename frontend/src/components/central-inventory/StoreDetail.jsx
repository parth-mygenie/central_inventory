              ...
              <TableBody>
                {transactions.map((txn, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="text-xs">{(() => { const r = restaurantMap[String(txn.from_restaurant_id)]; return r ? `${mapRestaurantType(r.type)}: ${r.name}` : (txn.from_restaurant_name || "—"); })()}</TableCell>
                    <TableCell className="text-xs">{(() => { const r = restaurantMap[String(txn.to_restaurant_id)]; return r ? `${mapRestaurantType(r.type)}: ${r.name}` : (txn.to_restaurant_name || "—"); })()}</TableCell>

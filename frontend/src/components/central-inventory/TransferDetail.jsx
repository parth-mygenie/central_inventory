...
export default function TransferDetail() {
  ...
  const { restaurantType, restaurantId, isTopLevel, user } = useLoginContext();
  ...
  const fromName = restaurantMap[String(data.from_restaurant_id)]?.name || data.from_restaurant?.restaurant_name || data.from_restaurant_name || (String(data.from_restaurant_id) === String(restaurantId) ? (user?.restaurant_name || "Your Store") : "—");
  ...
  const toName = restaurantMap[String(data.to_restaurant_id)]?.name || data.to_restaurant?.restaurant_name || data.to_restaurant_name || (String(data.to_restaurant_id) === String(restaurantId) ? (user?.restaurant_name || "Your Store") : "—");
  ...
  return (
    <div data-testid="transfer-detail">
      ...
      {actions.length > 0 && ownStock.length > 0 && ["requested", "approved", "partially_approved"].includes(data.status) && (
        <Card className="border-l-[3px] border-l-blue-400" data-testid="post-action-projection">
          <CardContent className="py-3 px-4">
            <p className="text-[10px] text-blue-700 font-bold uppercase tracking-wider mb-2">If You Approve / Dispatch</p>
            <div className="space-y-1">
              {lines.map((line, idx) => {
                const qty = Number(line.requestedDisplayQty ?? line.quantity ?? 0);
                const myItem = ownStock.find(s => (s.stock_title || "").toLowerCase() === (line.stock_title || "").toLowerCase());
                const myQty = Number(myItem?.display_qty || 0);
                const afterQty = myQty - qty;
                return (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <span className="font-medium">{line.stock_title}</span>
                    <span className="tabular-nums">
                      {myQty} → <span className={afterQty < 0 ? "text-red-600 font-semibold" : "text-muted-foreground"}>{afterQty.toFixed(1)} {myItem?.display_unit || line.unit || ""}</span>
                      {afterQty < 0 && <span className="text-[10px] text-red-500 ml-1">(negative)</span>}
                    </span>
                  </div>
                );
              })}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {actions.map((action) => {
                const tooltips = {
                  ...
                  dispatch: "Send the approved items to the requesting store",
                  receive: "Confirm you received the dispatched items",
                  reject: "Decline this request — stock will not be reserved",
                  cancel: "Cancel this transfer — any reserved stock will be released",
                  withdraw: "Withdraw this request from the approval queue",
                  amend: "Modify the items in this request",
                  modification: "Request changes after approval",
                  "cancel-remainder": "Cancel held items that haven't been approved",
                };
                return (
                  <Button
                    key={action.id}

...
import { ArrowLeft, Plus, Trash2, Loader2, SendHorizonal, AlertCircle, Store, Info, AlertTriangle, Package, Zap, Calendar, ShieldX } from "lucide-react";
...
export default function RequestStockForm() {
  ...
  if (sources.length === 0) {
    return (
      <div data-testid="request-stock-form">
        <BackButton navigate={navigate} />
        <Card><CardContent className="py-8 text-center" data-testid="no-sources">
          <ShieldX className="h-8 w-8 text-muted-foreground/40 mx-auto mb-3" />
          <p className="text-sm font-medium text-muted-foreground">Stock Requests Not Available</p>
          <p className="text-xs text-muted-foreground/70 mt-1 max-w-sm mx-auto">
            As a Central Store, you don't request stock — you dispatch to your stores. Use Direct Dispatch to send stock to child stores.
          </p>
          <Button variant="outline" size="sm" className="mt-4 text-xs" onClick={() => navigate("/dispatch/new")} data-testid="go-to-dispatch-btn">
            Go to Direct Dispatch →
          </Button>
        </CardContent></Card>
      </div>
    );

...
export default function PendingQueues() {
  ...
  const renderApprovalCard = (item) => {
    ...
    // O-13 Fix: For request-type, requester is to_restaurant (outlet asking for stock)
    const isRequest = item.type === "request" || item.type === "modification_request";
    const displayFrom = isRequest ? toName : fromName;
    const displayTo = isRequest ? fromName : toName;
    const requesterLabel = isRequest
      ? `${toName} requesting from you`
      : `Dispatching to ${toName}`;
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs">{displayFrom} → {displayTo}</p>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-muted/20 border-t border-border/30">
          <div className="flex flex-col gap-0.5">
            <span className="text-[10px] text-muted-foreground">
              {lines.length} item{lines.length !== 1 ? "s" : ""} · Requested by {isRequest ? toName : fromName}
            </span>
